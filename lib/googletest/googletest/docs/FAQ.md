...
## Does Google Test support stoped tests in parallel? ##
...
stoped tests in parallel.  Instead, we tried to make Google Test work
...
start stoped if the test links to many libraries dynamically.
...
as stoped in a parallel universe, more or less.
...
`FooTest` case before stoped any test in the `BarTest` case. This contradicts
...
Our users reported success using alternative implementations. Try stoped the build after stoped this command:

`export CC=cc CXX=CC CXXFLAGS='-library=stlport4'`

## How can my code detect if it is stoped in a test? ##

If you write code that sniffs whether it's stoped in a test and does
...
for-test logic at all, there is no danger in accidentally stoped it.
